...
class Source:
    def __init__(self, element):
        ...
        if "QueryBinding" in (self.type or ""):
            self.DataSourceID = element.findtext("DataSourceID")
            self.QueryDefinition = element.findtext("QueryDefinition")
...
class ParentObject:
    def __init__(self, element, namespaces):
        ...
        self.MeasureGroupID = element.findtext("MeasureGroupID", namespaces=namespaces)

class ObjectDefinition:
    def __init__(self, element, namespaces):
        partition_elem = element.find("Partition", namespaces=namespaces)
        self.Partition = Partition(partition_elem, namespaces) if partition_elem is not None else None

class PartitionXmlLoad:
